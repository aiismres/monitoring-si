...
import styles from './celldatesech2.module.css';
...
type Props = {
  ...
  param: Extract<
    SechKeys,
    'sdAs' | 'dopusk' | 'sdPas' | 'soglGtp' | 'planPodachi'
  >;
  sechData: SechData;
  // otAmount: number;
...
Props) {
  ...
  async function updSechData(newDate: Dayjs | null) {
    ...
    if (res?.ok) {
      ...
      console.log('Ошибка обновления данных /api/readsech');
      // setAppState((st) => ({
      //   ...st,
      //   isMsgOpen: true,
      //   isSuccess: false,
